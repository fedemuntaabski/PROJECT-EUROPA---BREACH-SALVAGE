...
## Movement Architecture Correction

- Replaced the hybrid movement model with strict room-bound spatial movement.
- Defined each room as a physical container with an explicit spatial boundary instead of relying on implicit logical room switching.
- Represented each room connection with a real exit zone that can be seen in debug playtests and detected by the movement system.
- Removed direct input-based room switching so input now only drives movement inside the current room.
- Kept transitions tied to exit-zone entry so the player only changes rooms through deliberate spatial traversal.
- Kept the camera follow logic independent from room switching so it tracks the player directly.
- Expanded the debug readout to show the current room ID, the player's room-local position, and the active exit zones.
